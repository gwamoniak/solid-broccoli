...
- [ ] Milestone 2: `spectro-core` library — sensor/spectrum domain types, analysis math, transport/codec abstractions, unit tests.
...
In `QML_ML_Camera/spectro-core/SensorDevice.h`: the abstract QObject base with `virtual bool connectDevice(QString* err)`, `disconnectDevice()`, `start()`, `stop()`, `setParams(const AcquisitionParams&)`, `QString name() const`, signals `errorOccurred(const QString&)`, `connectedChanged(bool)`. `SpectrometerDevice.h` derives from it adding the `spectrumReady(const Spectrum&)` signal; `GeigerDevice.h` (Milestone 10) adds `readingReady(const GeigerReading&)`. `CodecDevice(std::unique_ptr<SensorTransport>, std::unique_ptr<ProtocolCodec>)` implements the base generically and emits the typed signal matching whichever `SensorReading` alternative the codec delivers.
