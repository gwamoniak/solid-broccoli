...
- [ ] Milestone 12: Vision pipeline v1.1 — frame processors gain a structured result channel; object-detection plugin on ONNX Runtime (boxes on preview/recording, live detection list); acceptance with a real model gated on model presence, everything else hardware- and model-free.
- [ ] Milestone 13: On-device AI analyst — `SpectralLineLibrary` + `PeakMatcher` (deterministic identification), report context builder, llama.cpp-backed `AiAnalyst` service (default model Gemma 3 4B QAT Q4, fallback 1B), `reports` table + session-detail report viewer/export; generation acceptance gated on a downloaded GGUF model.
